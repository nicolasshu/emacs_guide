...
In Spacemacs, go to =dotspacemacs/layers= > =dotspacemacs-configuration-layers=, and set the =org-enable-roam-support= to true =t=. If you have only activated =org= as a base layer, you'll have to create parenthesis. Then reload your configuration file via =SPC f e R=.
...
In order to setup org-roam, you'll need to setup a few variables, and then reload your configuration with =SPC f e R=

#+begin_src elisp
  ;; Setup where the Org-Roam notes will be located
  ;;     Make sure to change it to the location "$REPO/08_orgroam/org-roam-notes"
  (setq org-roam-directory (file-truename "~/path/to/org-roam-notes"))

  ;; Make sure that the org-roam database is being updated
...
In Spacemacs, make sure that the variable =org-enable-roam-support= is set to =t=.
...
In order to use Org-Roam-UI, you can simply activate its mode via =M-x org-roam-ui-open= and it will be launched under http://127.0.0.1:35901.

[[./assets/org-roam-ui.png]]
